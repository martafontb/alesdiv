    ...
    if ( is_array($data)) {
        foreach ($data as $key => $value) {
            if (is_string($value)) {
	            ...
	            // Make sure not to put the protocol in links
	            $data[$key] = str_replace( 'http://', '//', $data[$key] );
            }
        }
    }
...
add_filter( 'of_options_before_generate_options_css', 'of_filter_load_media_upload');