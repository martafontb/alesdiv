...
class sidebar_generator {
		...
		if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar( array(
					...
					'id' => "sidebar-$i",
			    	'before_widget' => '<div id="%1$s" class="sbg_widget wr1 widget ' . esc_attr( $sidebar_class ) . ' %2$s">',
		   			'after_widget' => '</div>',
		...
		?>
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					mysack.setVar( 'sidebar_generator_nonce', '<?php echo wp_create_nonce('sbg_add_sidebar'); ?>' );
				...
				function remove_sidebar( sidebar_name,num )
				{
					...
					var mysack = new sack("<?php echo site_url(); ?>/wp-admin/admin-ajax.php" );    
				  	...
				  	mysack.setVar( "sidebar_name", sidebar_name );
					mysack.setVar( 'sidebar_generator_nonce', '<?php echo wp_create_nonce('sbg_remove_sidebar'); ?>' );
	...
	public static function add_sidebar(){
		...
		check_admin_referer( 'sbg_add_sidebar', 'sidebar_generator_nonce' );
		...
		$js = "
			...
			removeLink.setAttribute('onclick', 'remove_sidebar_link(\'$name\', ' + iteration + ' )');
	...
	public static function remove_sidebar(){
		...
		check_admin_referer( 'sbg_remove_sidebar', 'sidebar_generator_nonce' );
		...
		<div class="wrap">
			...
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<tr class="<?php echo esc_attr( $alt ); ?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_html($sidebar); ?>',<?php echo esc_html($cnt+1); ?>);" title="Remove this sidebar">remove</a></td>
				</tr>
				<?php
						$cnt++;
					}
