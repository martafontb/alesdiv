...
if ( $wp_3_5 ){
            ...
            if ( $attachments )
                foreach ( $attachments as $attachment_id ) {
                    echo '<li class="image" data-attachment-id="' . $attachment_id . '">
							...
							<ul class="actions"><li><a href="#" class="delete" title="' . esc_html__( 'Delete image', 'freelo' ) . '">' . esc_html__( 'Delete', 'freelo' ) . '</a></li></ul>
						</li>';
                }
            ?>
        </ul>
    ...
    <?php if ( isset($value) && '' != $value ) { ?>
        <a href="#" class="add_portfolio_images button-secondary"><?php esc_html_e( 'Add images', 'freelo' ); ?></a>
        <p class="additional_description"><?php 'Use <strong>Portfolio Gallery</strong> shortcode in content to display the gallery.'; ?></p>
    <?php } else { ?>
        <a href="#" class="add_portfolio_images button-secondary"><?php esc_html_e( 'Create Gallery', 'freelo' ); ?></a>
        <p class="additional_description"><?php 'Use <strong>Portfolio Gallery</strong> shortcode in content to display the gallery.'; ?></p>
    <?php } ?>
        ...
        jQuery(document).ready(function($){
            ...
            jQuery('.add_portfolio_images').click(function( e ) {
                ...
                portfolio_frame = wp.media.frames.downloadable_file = wp.media({
                    ...
                    title: '<?php esc_html_e( 'Add Portfolio images', 'freelo' ); ?>',
                    button: {
                        text: '<?php esc_html_e( 'Add to gallery', 'freelo' ); ?>'
                    },
                    multiple: true
                });
                        ...
                        if ( attachment.id ) {
                            ...
                            $portfolio_images.append(
                                '<li class="image" data-attachment-id="' + attachment.id + '">' +
                                    ...
                                    '<ul class="actions">' +
                                    '<li><a href="#" class="delete" title="<?php esc_html_e( 'Delete image', 'freelo' ); ?>"><?php esc_html_e( 'Delete', 'freelo' ); ?></a></li>' +
                                    '</ul>' +
                                    '</li>'
                            );
                        }
...
<?php
    ...
    // WORDPRESS < 3.5.0
    echo '<a href="#" id="ishyoboy_images_upload" class="button-secondary upload-' . $postid . '">' . esc_html__( 'Create Gallery', 'freelo' ) . '</a>';
    echo '<p class="additional_description">' . esc_html__( 'After setting the gallery images up, use <strong>[portfolio_gallery]</strong> in content to display them.', 'freelo' ) . '</p>';
    ?>
    <script type="text/javascript">
        jQuery(document).ready(function($){
