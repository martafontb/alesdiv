...
class WP_Import extends WP_Importer {
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'wordpress-importer' ) . '</strong><br />';
			printf( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'wordpress-importer' ), '<code>' . esc_html( $file['file'] ) . '</code>' );
			echo '</p>';
			return false;
		}
...
<?php
	...
	function process_menu_item( $item ) {
		...
		// MegaMenu Support - added by IshYoBoy
		foreach ( $item['postmeta'] as $meta ) {
			if ( false !== strpos($meta['key'], '-ish-') ){
				$meta['key'] = str_replace('-', '_', $meta['key'] );
			}
			${$meta['key']} = $meta['value'];
		}

		if ( ! isset($_menu_item_ish_megamenu) ) { $_menu_item_ish_megamenu = ''; }
		if ( ! isset($_menu_item_ish_division) ) { $_menu_item_ish_division = ''; }
		if ( ! isset($_menu_item_ish_textarea) ) { $_menu_item_ish_textarea = ''; }
		...
		$args = array(
			...
			// MegaMenu Support - added by IshYoBoy
			'menu-item-ish-megamenu' => $_menu_item_ish_megamenu,
			'menu-item-ish-division' => $_menu_item_ish_division,
			'menu-item-ish-textarea' => $_menu_item_ish_textarea,
		);
