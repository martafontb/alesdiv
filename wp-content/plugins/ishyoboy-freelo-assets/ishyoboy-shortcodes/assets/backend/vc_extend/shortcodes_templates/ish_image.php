...
if ( ! empty( $thumbnail ) ){
	...
	if ( '' != $sc_atts['link_type'] ){
		...
		if ( 'custom' == $sc_atts['link_type'] ){
			...
			$return .= ( '' != $url_data['url'] ) ? ' href="' . esc_url( apply_filters( 'ishfreelotheme_sc_url', $url_data['url'] ) ) . '"' : '';
			...
			// IMAGE POPUP CAPTION
			$img_caption = '';
			if ( ( 'popup' == $sc_atts['show_caption'] ) || ( 'both' == $sc_atts['show_caption'] ) ) {
			...
			$return .= ( '' != $img_details[0] ) ? ' href="' . esc_url( $img_details[0] ) . '" title="' . esc_attr( $img_caption ) . '" target="_blank"' : ' href="#"';
		}
	...
	if ( ( 'image' == $sc_atts['show_caption'] ) || ( 'both' == $sc_atts['show_caption'] ) ) {
		$img_obj = get_post( $sc_atts['image'] );
