...
if ( '' != $sc_atts['size'] && is_numeric( $sc_atts['size']) ) {
	$icon_width = $sc_atts['size'];
	$icon_size = $sc_atts['size'];
}
// Take into consideration width only if icon type is simple - change dependency if logic changed
if ( '' != $sc_atts['width'] && is_numeric( $sc_atts['width']) && 'simple' == $sc_atts['type'] ) {
	$icon_width = $sc_atts['width'];
}
...
$class .= ( '' != $sc_atts['color'] && ( false === strpos( $sc_atts['color'], '#' ) ) ) ? ' ish-' . esc_attr( $sc_atts['color'] ) : '' ;
$class .= ( '' != $sc_atts['text_color'] && '' == $sc_atts['bg_color'] && ( false === strpos( $sc_atts['text_color'], '#' ) ) ) ? ' ish-text-' . esc_attr( $sc_atts['text_color'] ) : '' ;
$class .= ( '' != $sc_atts['active_text_color'] && ( false === strpos( $sc_atts['text_color'], '#' ) ) ) ? ' ish-active-text-' . esc_attr( $sc_atts['active_text_color'] ) : '' ;
...
if ( '' == $sc_atts['bg_color'] ) {
	if ( '' != $sc_atts['style'] || '' != $sc_atts['icon_url'] || 'square' == $sc_atts['type'] || 'circle' == $sc_atts['type'] || 'simple' == $sc_atts['type'] || 'hexagon' == $sc_atts['type'] || 'hexagon_rounded' == $sc_atts['type'] ){
		...
		if ( '' != $sc_atts['size'] && is_numeric( $sc_atts['size'] ) ){
			$return .= 'font-size:' . esc_attr($icon_size) . 'px;';
			$return .= 'width:' . esc_attr($icon_width) . 'px;';
			$return .= 'height:' . esc_attr($icon_size) . 'px;';
		}
		$return .= '"';
	}
}
...
// URL OPEN
if ('' != $url_data['url'] ) {
	$return .= '<a href="' . esc_attr( apply_filters( 'ishfreelotheme_sc_url', $url_data['url'] ) ) . '"';
...
if ( '' != $sc_atts['icon_url'] ) {
	...
	if ( '' != $sc_atts['text_color'] || '' != $sc_atts['bg_color'] ) {
		$return .= ' style="';

		if ( '' != $sc_atts['text_color'] ) {
			$return .= 'color: ' . esc_attr( ishfreelotheme_colors_to_hex( $sc_atts['text_color'] ) ) . ';';
		}
		if ( '' != $sc_atts['bg_color'] ) {
			$return .= 'background-color: ' . esc_attr( ishfreelotheme_colors_to_hex( $sc_atts['bg_color'] ) ) . ';';
		}
		$return .= '"';
	}
...
if ('' != $sc_atts['size'] ) {
	...
	switch ($sc_atts['type']) {
		case 'square':
			$return .= 'width:' . esc_attr($icon_width) . 'px;';
			$return .= 'height:' . esc_attr($icon_size) . 'px;';
			$return .= 'font-size:' . ( esc_attr($icon_size) / 1.7 ) . 'px;';
			break;
		...
		case 'hexagon_rounded':
			$return .= 'width:' . esc_attr($icon_width) . 'px;';
			$return .= 'height:' . esc_attr($icon_size) . 'px;';
			$return .= 'font-size:' . ( esc_attr($icon_size) / 2.1 ) . 'px;';
			break;
	...
	$return .= 'line-height:' . esc_attr($icon_size) . 'px;';
