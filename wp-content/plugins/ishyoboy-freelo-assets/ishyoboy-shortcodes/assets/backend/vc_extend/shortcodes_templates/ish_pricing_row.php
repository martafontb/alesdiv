...
switch ($defaults['type']){
	case 'icon':
		...
		foreach ( $defaults as $key => $val ){
			if ( !in_array( $key, array( 'type', 'icon', 'icon_size', 'ti_text_color' ) ) )
				$defaults[$key] = '';
		}
	...
	case 'button' :
		...
		foreach ( $defaults as $key => $val ){
			if ( !in_array( $key, array( 'type', 'button_text', 'url', 'button_size', 'color', 'text_color', 'border', 'button_icon', 'button_icon_align' ) ) )
				$defaults[$key] = '';
		}
	...
	default :
		foreach ( $defaults as $key => $val ){
			if ( ! in_array( $key, array( 'type', 'text_content', 'text_size', 'ti_text_color' ) ) )
				$defaults[$key] = '';
		}
}
...
switch ( $sc_atts['type'] ){
	case '' :
		...
		$return .= ( '' != $sc_atts['ti_text_color'] ) ? '<span class="ish-highlight ish-text-' . esc_attr( $sc_atts['ti_text_color'] ) . '">' : '' ;
		$return .= do_shortcode($sc_atts['text_content']);
		$return .= ( '' != $sc_atts['ti_text_color'] ) ? '</span>' : '' ;
		$return .= '</' . $el_tag . '>';
		break;
	...
	case 'icon' :
		...
		$return .= '<div class="ish-sc-element ish-sc_icon';

		$return .= ( '' != $sc_atts['ti_text_color'] ) ? ' ish-text-' . esc_attr( $sc_atts['ti_text_color'] ) : '' ;
	...
	case 'button' :
		...
		$class .= ('no' != $sc_atts['border'] ) ? ' ish-border' : '' ;
		$return .= $class;
		$return .= '"' ;

		// URL
		$url_data = vc_build_link( $sc_atts['url'] );
		$return .= ( '' != $url_data['url'] ) ? ' href="' . esc_attr( apply_filters( 'ishfreelotheme_sc_url', $url_data['url'] ) ) . '"' : '';
